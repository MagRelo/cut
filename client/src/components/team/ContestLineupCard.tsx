 ...
 */
const sortPlayersByPosition = (players: PlayerWithTournamentData[]): PlayerWithTournamentData[] => {
...
const RoundIcon: React.FC<RoundIconProps> = ({ currentRound, leaderboardPosition }) => {
  ...
  return (
    <div className="flex-shrink-0 flex items-center h-8">
      {currentRound?.data.icon ? (
        <span className="text-xl text-gray-600 font-bold">{currentRound.data.icon}</span>
      ) : (
...
interface ContestLineupCardProps {
  contestLineup: ContestLineup;
  roundDisplay?: string;
  tournamentStatus?: string;
}

export const ContestLineupCard: React.FC<ContestLineupCardProps> = ({
  contestLineup,
  roundDisplay,
  tournamentStatus,
}) => {
  ...
  const getLineupRatio = () => {
    const ratios =
      contestLineup.tournamentLineup?.players.map((player: PlayerWithTournamentData) => {
    ...
    return ratios.reduce((sum: number, ratio: number) => sum + ratio, 0) / ratios.length;
  };
        ...
        <div className="px-4 py-3 pb-2 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              {/* <span className="text-xl text-gray-400 font-bold mr-4">
        ...
        <div className="p-1 bg-gray-50">
          <div className="flex justify-between items-center gap-x-4 px-3">
                ...
                <div className="flex items-center w-full p-1 rounded">
                  {isTournamentActive && (
                    <>
                      ...
                      <span className="text-sm font-medium text-gray-400 pr-1 ml-1">LINEUP</span>
                    </>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center space-x-2 h-8">
              {[...Array(4)].map((_, index) => {
                const sortedPlayers = sortPlayersByPosition(
            ...
            <div className="flex-1 min-w-0 text-center flex items-center h-8">
              <div className="w-full h-2 bg-gray-200 rounded-full relative">
                ...
                />
                <span className="sr-only">{Math.round(getLineupRatio() * 100)}% complete</span>
              </div>
            </div>
          </div>
          ...
          {expanded && (
            <div className="mt-2" onClick={(e) => e.stopPropagation()}>
              <PlayerTable
                players={sortPlayersByPosition(contestLineup.tournamentLineup?.players || [])}
