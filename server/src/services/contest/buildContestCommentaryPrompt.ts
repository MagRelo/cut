...
export function buildContestCommentaryPrompt(
  ...
  return [
    ...
    ...voice.instructions,
    "Open by using tournamentProgress to note colloquially how much golf remains, then establish the current race clearly enough to orient the audience. Translate the phase naturally—for example, “with the leaders yet to tee off,” “as the leaders approach the turn,” “with the leaders on the back nine,” or “coming down the closing stretch.” Do not quote leaderHolesRemaining unless an exact hole count is genuinely useful.",
    "Keep that opening context concise before prioritizing routes to winning over repeated position updates. Use lineupRoutes to explain which golfers each lineup needs to outperform, which teams share the same dependency, and whether a path is broad, narrow, or miracle-level.",
